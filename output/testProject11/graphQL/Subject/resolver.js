...
export default {
  ...
  Mutation: {
    ...
    async updateSubject(root, args, context, ast) {
      let gqlPacket = { root, args, context, ast, hooksObj };
      let { db, session, transaction } = await resolverHelpers.startDbMutation(gqlPacket, "Subject", SubjectMetadata, { update: true });
      return await resolverHelpers.runMutation(session, transaction, async() => {
    ...
    async deleteSubject(root, args, context, ast) {
      ...
      let gqlPacket = { root, args, context, ast, hooksObj };
      let { db, session, transaction } = await resolverHelpers.startDbMutation(gqlPacket, "Subject", SubjectMetadata, { delete: true });
      try {
